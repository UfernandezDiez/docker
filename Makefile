...
PROJECT_NAME=docker
STACK_NAME=docker-ufernandezdiez-korpoweb-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://docker.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t ufernandezdiez/docker-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t ufernandezdiez/docker-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name docker-backend-acceptance -d ufernandezdiez/docker-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name docker-frontend-acceptance --link docker-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ufernandezdiez/docker-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop docker-frontend-acceptance
	@docker stop docker-backend-acceptance
